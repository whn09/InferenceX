#!/usr/bin/env bash
#
# Kimi-K2.5 FP4 PD Disaggregated Serving Benchmark (Mooncake EFA)
# Runs on 2x AWS p6-b300.48xlarge instances.
#
# Prerequisites:
#   - P6-1 (PREFILL_HOST): vLLM kv_producer running on PREFILL_PORT
#   - P6-2 (DECODE_HOST):  vLLM kv_consumer running on DECODE_PORT
...
#   bash kimik2.5_fp4_b300_vllm-mooncake-disagg.sh
#
set -euo pipefail
...
echo "============================================"
echo " PD Disagg Benchmark (Mooncake EFA, B300)"
echo "============================================"
...
for CONC in $CONC_LIST; do
    NUM_PROMPTS=$(( CONC * 10 ))
    NUM_WARMUPS=$(( CONC * 2 ))
    RESULT_FILENAME="kimik2.5_fp4_pd_disagg_mooncake_efa_tp4_isl${ISL}_osl${OSL}_conc${CONC}_b300"
...
echo " ISL=${ISL} / OSL=${OSL}, TP=4, B300"
echo "============================================"
echo ""
echo "| Conc | Output tok/s | tok/s/gpu (4 GPUs) | Mean TPOT (ms) | Mean TTFT (ms) |"
echo "|:----:|:------------:|:------------------:|:--------------:|:--------------:|"
...
print(f'| {sys.argv[2]:^4} | {tput:>12.1f} | {tput/4:>18.1f} | {tpot:>14.2f} | {ttft:>14.2f} |')
